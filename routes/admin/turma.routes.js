  ...
  async (req, res) => {
    try {
      ...
      const result = await pool
        .request()
        .query(
          `SELECT id_turma, ano_letivo, cod_turma, turno FROM turma ORDER BY ano_letivo DESC, cod_turma`,
        );
      res.json({ sucesso: true, turmas: result.recordset });
    } catch (err) {
  ...
  async (req, res) => {
    ...
    try {
      ...
      const anterior = await new sql.Request(transaction)
        .input("id_turma", sql.Int, id)
        .query(`SELECT cod_turma FROM turma WHERE id_turma = @id_turma`);
      if (anterior.recordset.length === 0)
      ...
      await new sql.Request(transaction)
        .input("id_turma", sql.Int, id)
        .query(
          `DELETE n FROM notas n INNER JOIN turma_disciplina td ON td.id_turma_disciplina = n.fk_turma_disciplina WHERE td.fk_turma = @id_turma`,
        );
      await new sql.Request(transaction)
        .input("id_turma", sql.Int, id)
        .query(`DELETE FROM turma_disciplina WHERE fk_turma = @id_turma`);
      await new sql.Request(transaction)
        .input("id_turma", sql.Int, id)
        .query(`UPDATE alunos SET fk_turma = NULL WHERE fk_turma = @id_turma`);
      await new sql.Request(transaction)
        .input("id_turma", sql.Int, id)
      ...
      await registrarAuditoria({
        ...
        tabela: "turma",
        idRegistro: id,
        descricao: `Removeu a Turma ${anterior.recordset[0].cod_turma} e desvinculou os alunos`,
      });

      res.json({ sucesso: true, mensagem: "Turma removida com segurança." });
    } catch (err) {
