	...
	lista.forEach((p) => {
      ...
      <article class="record-row">
        ...
        <div class="record-actions" style="display: flex; gap: 8px;">
          <button class="action-btn edit" data-action="editar" data-id="${p.id_professor}">
            <i class="fa-solid fa-pen-to-square"></i> Editar
          </button>
          <button class="action-btn delete" data-action="excluir" data-id="${p.id_professor}" data-nome="${p.nome_completo}" style="background: #dc3545; color: white; border: none; padding: 6px 12px; border-radius: 6px; cursor: pointer;">
...
btnSalvarProfessor.addEventListener("click", async () => {
	...
	if (!email || !nome_completo || !data_nacimento) {
		showToast(
			"Por favor, preencha as credenciais e as informações pessoais.",
			"warning",
		);
		return;
	}
	...
	if (!modoEdicao) {
		...
		if (!cpf) {
			showToast("Digite o CPF para o novo professor.", "warning");
			return;
		}
	}
	...
	try {
		...
		showToast("Ficha do docente salva com sucesso!", "success");
		fecharModal();
		await buscarProfessores();
	} catch (e) {
	...
	if (action === "editar") {
		...
		try {
			const res = await requisicaoApi(
				`${urlBase}api/admin/professor/${id}`,
			);
			const d = await res.json();
			if (d.sucesso) {
				const p = d.professor;
			...
			showToast("Erro ao ler dados do servidor.", "error");
		}
	} else if (action === "excluir") {
		professorIdExcluindo = id;
...
function configurarEventosExclusao() {
	btnCancelarExclusao.addEventListener("click", () => {
		...
		professorIdExcluindo = null;
	});

	btnConfirmarExclusao.addEventListener("click", async () => {
		if (!professorIdExcluindo) return;
		try {
			const res = await requisicaoApi(
				`${urlBase}api/admin/professor/${professorIdExcluindo}`,
				{
					method: "DELETE",
				},
			);
			...
			if (d.sucesso) {
				showToast("Docente removido com sucesso!", "success");
				confirmDeleteOverlay.classList.remove("active");
				professorIdExcluindo = null;
				await buscarProfessores();
			} else {
				showToast(
					d.erro || "Erro ao tentar remover o docente.",
					"error",
				);
			}
		} catch (err) {
